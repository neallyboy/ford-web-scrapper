...
def create_escape_prices_df():
    ...
    escape_mfr_prices_df = pd.DataFrame(
        ford_mfr_escape_prices, columns=["Car Model", "Ford Manufacturer Price"]
    )
    escape_dealer_prices_df = pd.DataFrame(
        ford_dealer_escape_prices, columns=["Car Model", "Ford Dealer Price"]
    )

    # Merge datasets on 'Car Model'
    merged_df = pd.merge(
        escape_mfr_prices_df,
        escape_dealer_prices_df,
        on="Car Model",
        how="outer",
...
def create_escape_image_df():
    ...
    hero_image_df = pd.DataFrame(
        {
            "Model Hero Image": ["Escape"],
            "Ford Manufacturer Image": [ford_mfr_escape_image],
            "Ford Dealer Image": [ford_dealer_escape_image],
        }
    )
